...
export function TopBar() {
  ...
  const dateAr = now.toLocaleDateString('ar-SA-u-nu-latn', { weekday: 'long', day: 'numeric', month: 'long' })
  ...
  return (
    <header className="sticky top-0 z-30 px-4 pt-4 lg:ps-[260px]">
      <div className="flex h-[64px] items-center gap-3 rounded-2xl border border-[--color-line] bg-[--color-surface]/70 px-4 backdrop-blur-2xl">
        {/* mobile brand */}
        ...
        <div className="hidden md:flex items-baseline gap-2">
          <span className="text-[15px] font-extrabold text-[--color-ink]">مركز القيادة</span>
        ...
        {/* search */}
        <div className="ms-auto hidden md:flex h-10 min-w-[280px] items-center gap-2 rounded-xl border border-[--color-line] bg-black/30 px-3 transition-colors focus-within:border-[--color-admiral-2]/50">
        ...
        {/* AI assistant chip */}
        <button className="hidden md:inline-flex h-9 items-center gap-1.5 rounded-xl border border-[rgba(78,163,255,0.22)] bg-gradient-to-l from-[#0a3a7e]/40 to-[#003d82]/20 px-3 text-[12px] font-bold text-[--color-ink] transition-shadow hover:shadow-[0_0_24px_rgba(78,163,255,0.25)]">
          <Sparkles size={13} className="text-[--color-admiral-glow]" />
          مساعد سَفِي
        </button>

        {/* flag + clock */}
        <div className="hidden md:flex items-center gap-2.5 rounded-xl border border-[--color-line] bg-black/30 px-3 py-1.5">
          <SaudiFlag />
        ...
        {/* date AR (mobile) */}
        <div className="hidden sm:block md:hidden text-[12px] font-bold text-[--color-ink-2]">{dateAr}</div>

        {/* bell */}
        <button
          aria-label="notifications"
          className="relative grid h-10 w-10 place-items-center rounded-xl border border-[--color-line] bg-black/30 text-[--color-ink-2] transition-colors hover:text-[--color-ink]"
        >
          <Bell size={16} />
        ...
        {/* user */}
        <div className="flex items-center gap-2.5 rounded-xl border border-[--color-line] bg-black/30 px-2 py-1.5">
